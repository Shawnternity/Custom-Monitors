...
##### Script to raise a health event for the slowesr Outlook plugin (needs to take lomger than threshold)
...
try
{
    ...
    #Boot Time threshold to raise a health event in seconds
    $Threshold =1
	
	#Initialize maxvalue veriable to capture slowest plugin
	$maxvalue=0
	...
	#Set time span to 30 minutes before current time
	$lastpolltime=(get-date).addminutes(-30)
	
	#Pattern to extract parameter from message
	$regex ="Name: (?<name>.*)\n.*\n.*\n.*\n.*\n.*\n.*\n.*Boot Time.*:\s+(?<number>\d+)"
	
	#Get events via get-win-event cmdlet and filter with regex pattern
	$event = get-winevent -FilterHashtable @{logname='application';providername='Outlook';id=45; StartTime=$lastpolltime } | Select -ExpandProperty message -ErrorAction SilentlyContinue
	$results = [Regex]::Matches($event,$regex)
	
	#No need to keep the whole cache of events in memory
    $event = ""

	#Iterate through resuts to find slowest plugin
    $plugins = @()
	
	foreach($plugin in $results)
	{

		$value=[int]$plugin.Groups["number"].value/1000
		$name=$plugin.Groups["name"].value
		...
		#If  Boot Time is higher than threshold, set the the health event boolean to true
		if ($maxvalue -gt $Threshold)
		{
		 ...
		 # Check whether a custom health event should be created by the Agent
			...
			[ActionExtensionsMethods.PowershellPluginMethods]::SetApplicationName("Microsoft Outlook")
			[ActionExtensionsMethods.PowershellPluginMethods]::SetError("Boot Time over " + "$Threshold" + " seconds")
			[ActionExtensionsMethods.PowershellPluginMethods]::SetComponent("Outlook Plugin")
			[ActionExtensionsMethods.PowershellPluginMethods]::SetSubComponent("$maxname")
			[ActionExtensionsMethods.PowershellPluginMethods]::SetDetails("Boot Time " + "$maxvalue" + " seconds" )
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUM0zBmN5bQi0j6ywM7Pava5V3
# YmKgghMOMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# FLbjTUpSudDbYGoRc35FjhzkPt1cMA0GCSqGSIb3DQEBAQUABIIBAIdIJziZJs8l
# lRb/a5XFqBynGHAwL1jwlemGvRDbHIiiGJ013QgoLw8luJmImSELh59gGXVvvbIh
# Qew7YcqTH1DaNWwz98zEEJ8wjWlyWrjiNDcsApFB/jJvh/vy1jGbaNWaz4MLSa4P
# 6IGQdFw1Mx4TXu7qKbU3Ws365rcdBCuDjt/VdPUsIASq9s0H4Lk1tA3s677PhwTR
# MCxlRdvReS8h1QaBvnv+lhrUeVcngr2tgsUZLJkzjQAZRPufmE68hU1jhrWDIh97
# THoGb4kCuEAlYRLw7Ic/9hKoVD2N9eDr0I58fQ/ktho6+fIiDCMouVjeBSuN3OjM
# 2Zt6mdBsJCyhggILMIICBwYJKoZIhvcNAQkGMYIB+DCCAfQCAQEwcjBeMQswCQYD
...
# ATAcBgkqhkiG9w0BCQUxDxcNMjAwMjI2MTQzMzU0WjAjBgkqhkiG9w0BCQQxFgQU
# s+QdkVXOW+eJB3JNSrB+EAuX5S0wDQYJKoZIhvcNAQEBBQAEggEAgTK7Ydbj9zRZ
# i1jjxUe4zBQBDDdr11J6cjaD9TSmP4QLeosxwhZ/sX9XRE+jfumjpT8mlSDo6Tza
# rWrwZTsKWWRh9QXMo9PECiHGJD1wjGqH1KkaW6qmz3QS0fauERVdDPafa6Ypxn75
# O1z34YnVJpT5ktoBlZKTD3rhtJo47LJPv0uaJmAiEp3f2xBs5RceE69gGgPCDR3n
# /giT1rXHuuojyWBBI1a+xyTz9wZ1l+NS5uD0Ao+M4yo3xfPHrsoJDPEoox3YVOPL
# AZIgBb+TRx6TioNxsy/6r8rdb9gS+9hncfETMaczQaupqL4LCCl/B6Y8KdgmyGX4
# lZ8md0aAPw==
